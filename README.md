...
- 따라서 몽고DB Unique 인덱스를 활용하여 userId + eventId + status(성공, 실패) 조합에 대해 중복 데이터가 저장되는 것을 DB 레벨에서 강력하게 차단하였습니다. 또한 저장 로직에 try/catch를 활용하여 이미 보상 신청 이력이 있다는 메시지도 추가하였습니다.
